...
int main(int argc, const char *argv[]) 
{
    ...
    if(pthread_create(&client_daemon, NULL, client_refresher, &client)) {
        ...
        exit_example(EXIT_FAILURE, NULL, NULL);
    ...
    /* exit */ 
    exit_example(EXIT_SUCCESS, NULL, &client_daemon);
}
...
void reconnect_client(struct mqtt_client* client, void **reconnect_state_vptr)
{
    ...
    BIO* sockfd = open_nb_socket(reconnect_state->hostname, reconnect_state->port);
...
void* client_refresher(void* client)
{
    while(1) 
    {
        int sync_state = mqtt_sync((struct mqtt_client*) client);
        if (sync_state != MQTT_OK) {
            usleep(2000000U);
        } else {
            usleep(100000U);
        }        
